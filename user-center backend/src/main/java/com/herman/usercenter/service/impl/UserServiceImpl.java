...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword, String planetCode) {
        ...
        if (StringUtils.isNotBlank(planetCode) && planetCode.length() > 5) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "Legacy code must not exceed 5 characters");
        }
        // 账户不能包含特殊字符
        String validPattern = "[`~!@#$%^&*()+=|{}':;',\\\\[\\\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]";
        ...
        if (StringUtils.isNotBlank(planetCode)) {
            ...
            if (count > 0) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "This legacy code is already in use");
            }
        }
        // 2. 加密
